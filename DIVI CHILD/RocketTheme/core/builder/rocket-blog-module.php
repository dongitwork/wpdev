...
class ET_Builder_Module_Rocket_Blog extends ET_Builder_Module_Type_PostBased {
	function init() {
		$this->name       = esc_html__( 'Rocket Blog', 'et_builder' );
		$this->slug       = 'et_pb_rkt_blog';
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if ( have_posts() ) {
			...
			// Rocket Hook post shows page blog
			while ( have_posts() ) {
				the_post();
		...
		$output = sprintf(
			...
			%4$s',
			( 'on' === $fullwidth ? 'et_pb_posts rkt_blogs_list' : 'et_pb_blog_grid clearfix' ),
			$posts,
			esc_attr( $class ),
...
new ET_Builder_Module_Rocket_Blog;
